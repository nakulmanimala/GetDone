...
const projectColors: Record<string, string> = { Personal: '#a78bfa', DevOps: '#38bdf8', 'Nakul To-Do': '#34d399', Inbox: '#8a8f98' }
    ...
    <div className="app-shell">
      <aside className={`sidebar ${sidebarOpen ? 'sidebar-open' : ''}`}>
        <div className="brand-row">
          ...
          <div><strong>Momentum</strong><span>Nakul's workspace</span></div>
        ...
        <div className="project-list">
          {['Personal', 'DevOps', 'Nakul To-Do'].map((project) => <button key={project}><i style={{ background: projectColors[project] }} />{project}<span>{tasks.filter((task) => task.project === project && task.status === 'open').length}</span></button>)}
        </div>
      ...
      {selected && <aside className="detail-panel">
        ...
        <div className="detail-body">
          ...
          <label>Project<select value={selected.project} onChange={(event) => setTasks(updateTask(tasks, { ...selected, project: event.target.value }))}>{['Inbox', 'Personal', 'DevOps', 'Nakul To-Do'].map((project) => <option key={project}>{project}</option>)}</select></label>
